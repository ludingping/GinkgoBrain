# A 股日频 趋势底座 + 横截面增量（2026-09-21）

目标（用户定）：跑赢中证全指等权且回撤更小。设计 v0.1：GinkgoRoad docs/GinkgoBrain/A股日频-趋势底座与横截面增量-设计.md

- [x] 数据盘点：全市场日线自 2015-12（2590 → 5235 只）；无指数、无历史市值/ST/上市日期；资金流 2025-04 起
...
# H4b' 兄弟资产检验（2026-09-17）

- [x] Spider 回填 ETH/SOL/BNB 比率历史至 192.168.1.68（各 ≈1.78M 行，失败日 0；归档起点 2021-12-01）
- [x] 登记 H4b'（两级标准）——先于看兄弟资产数据
- [x] 探针 `--min-ratio-coverage`；ETH/SOL/BNB gate-on k=18/42；`reports/h4b2_sibling_probe.md`
- [x] 判定：两级 FAIL（BNB 最近折 −0.23）→ 持仓/杠杆数据方向整体关闭
- [ ] 后续：三臂模拟盘观察 ≥ 8 周；H1–H8 全部关闭后，新假设需要新的数据源或新的问题

## 审查
- 内存看门狗会杀掉后台的探针（峰值 3.6 GB + 回填并行）；探针改前台跑。
- 同一特征、同一时段，BTC +0.31 与 BNB −0.23：单资产强 IC 不是证据，这条应写进探针门槛的解释里。

---

# H4a/H4b/H5 持仓信号探针（2026-09-17，收益增强线）

- [x] 设计文档登记（方向、门槛、gate-on 主判定、预判）
- [x] `read_position_ratio` 接入探针：`resample_position_ratio`（桶末值 + taker 均值）、指标后合并、覆盖 <80% 报错、按配置时区切桶；探针特征 4 个；tests +2
- [x] 4h gate-on / 4h 全样本 / 1d gate-on / 1d 全样本；`reports/h4_h5_position_ratio_probe.md`
- [x] 判定：三条全 FAIL（反号）；结构性观察：ETF 后 retail_minus_top 正向延续
- [ ] 待用户：是否登记反向假设 H4b'，用其他币种比率（若已入库）做未触碰检验

## 审查
- 与 H1 同一模式：2023-11 之后情绪/拥挤指标反号。两次独立数据源得到同一结构性观察，比任何单个 IC 更有价值。
- 1d gate-on 的 n 太小（158–364），日频信号在 BTC 上根本没有足够样本做三折。

---

# H8 震荡感知退出（2026-09-17，预登记后同日跑完）

- [x] 设计文档登记 H8（穿越计数定义、3×3 网格、四条标准、预判不过）——先于看数据
- [x] `backtest_signal_layered.daily_sma_cross_count` + `backtest_rules` 规则 `gate_exit_chop`；tests +2
- [x] BTC/ETH 网格、SOL/BNB 主变体、机制检验（episode 内回撤）；`reports/h8_chop_aware_exit.md`
- [x] 判定：四条全 FAIL，多数格子比 V1 更差 → "分行情阶段配参数"思路关闭
- [ ] 待用户：推送 Brain 与 Spider（第三臂部署需要 Spider develop 上远端）；H4b 三折探针

## 审查
- 探测器识别震荡的时点（2024-09）晚于损失发生（2024-04→08），关闭退出的时段正好覆盖 2025-01 趋势下跌开头。这不是阈值问题，是"事前判断阶段"这类规则的结构性滞后。
...
- [x] 14. **H3'b → V1 PASS（见审查）**：V1 SMA50→0% / V2 SMA20→50% / V3 SMA100→50%，各跑一次；test 用无害判定；全败则收益下限放宽到 60% 重判
...
### H3'b（E1 变体，预注册后执行，2026-09-07）：**V1 通过**

§1.1 已按用户决定修订：test 段对退出规则用"无害"判定；后备（收益下限 60%）未启用。
报告：`reports/backtest_rules_h3b_exit_variants.md`、确认段 `reports/backtest_rules_h3b_v1_confirm_pre_val.md`。

| val 2024-10→2026-04 | 总收益 | 最大回撤 | 回撤比 | Calmar | 交易/年 | 判定 |
|---|---|---|---|---|---|---|
| gate_only | +17.6% | −34.0% | — | 0.33 | 12 | — |
| E1 SMA50 → 50%（参照） | +29.2% | −26.0% | 0.77 | 0.70 | 27 | FAIL（回撤比） |
| **V1 SMA50 → 0%** | **+40.0%** | **−19.0%** | **0.56** | **1.29** | 16 | **PASS** |
| V2 SMA20 → 50% | +16.4% | −28.3% | 0.83 | 0.37 | 35 | FAIL（回撤比、换手） |
| V3 SMA100 → 50% | +32.9% | −25.9% | 0.76 | 0.79 | 24 | FAIL（回撤比） |

test 2026-04→09：四条规则均未触发，与 gate_only 相同（+17.2% / −4.6%），无害判定全部 PASS。

**确认段**（2021-08→2024-10，3.2 年，含 2022 熊市；从未参与任何规则选择，只跑一次）：

| | 总收益 | 最大回撤 | 回撤比 | Calmar | 交易/年 |
|---|---|---|---|---|---|
| buy&hold | +53.7% | −77.0% | | 0.19 | |
| gate_only | +70.4% | −37.6% | — | 0.49 | 9 |
| **V1** | **+110.9%** | **−25.6%** | 0.68 | **1.04** | 13 |

- V1 在两个独立区间上方向一致：收益 +127% / +57%，回撤比 0.56 / 0.68，Calmar 翻 2–4 倍，换手 ≤ 16。确认段回撤比差 0.01 到 2/3 线，但这不是选择区间，方向已足够。
- **清仓优于减半**：跌破 SMA50 后留 50% 仓位的那一半在 2025 上半年吃掉了大部分回撤；退出信号一旦触发就该走完。
- 规则完整形式：`target = 0 if not gate_on(SMA200) else (0 if close_d < SMA50_d else 1.0)`，两个门都用最近已收盘 UTC 日线。等价于"双均线过滤的 100%/0% 趋势跟随"。
- 局限：test 段没有触发，OOS 证据只是"无害"；val 的改善集中在 2025-01→04 一次回撤；H3'+H3'b 在 val 上共看了 6 条规则（参数全部先验，无搜索）。

### 下一步
1. **Spider 部署 V1**（需要新 artifact 契约：`serving` 里加 `exit_sma_days: 50`，与 `regime_ok` 同源的日线数据；Spider 已拉 1d K 线算 SMA200，SMA50 零成本）。建议先 A/B：同账户结构跑 gate_only 与 gate+SMA50。
2. 在 val/pre_val 上出 V1 与 gate_only 的权益曲线与回撤曲线图，供人工检查触发点。
3. H2a（OI 高位）作为收益增强候选，只在 V1 上线并观察数周后再叠加，且要加滞回。

### 旧候选（H3' 前的决定项，已由 H3'b 解决）
