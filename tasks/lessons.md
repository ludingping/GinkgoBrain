# Lessons

## 2026-09-05 · BTC 4h PPO v2（对齐 Spider 虚拟盘）

- **看 train/eval 曲线的相对走向，不看绝对值。** 训练奖励单调升、eval 单调降且 best_model 停在第一个 checkpoint = 纯过拟合；8k 根 bar 上 1M 步（122 遍）必然如此。小样本先算"每根 bar 被看多少遍"，超过 ~20 遍就该缩网络/减 epoch/早停。
- **一段区间的好成绩不算证据。** 首次 1M 模型 test +12% 但 val −17.5%；牛市里偏多就是 beta。至少两个 regime 不同的留出段都要看。
- **服务侧能算出来的信号才有资格进池。** `sig_mtf_1d_regime` 需要 ~960 根 4h 历史，Spider 只拉 500 根 → 线上恒为 0；日线还有 UTC/Shanghai 边界差异。信号筛选时用 `--exclude` 把服务侧约束前置。
- **零方差列会让相关性筛选静默出错**（corr=NaN → 排序未定义）。任何进候选池的列先检查 std。
- **导出物要带契约，不只带模型。** `signals.yaml` 里的 `serving:`（obs/state 定义、锁、成本）+ `fingerprint:`（golden 输入与期望输出）让另一个仓库能自检 train/serve 一致性；否则像 v1 那样 obs 错位一根 bar 也没人发现。
- **旧配置的 `end_date` 会让新留出集切片为空**——`backtest_signal_layered.py` 现在会明确报错，但用旧配置比对时记得延长 `end_date`。

## 2026-09-05 · BTC 1h PPO v3

- **配置深合并会静默继承默认值。** `train.py` 用 `config/default.yaml` 做底再 deep_merge stage 配置；任何进入 `_ALLOWED_ENV_KEYS` 的键若 stage 里没写就继承默认（`trade_penalty_coef: 1.0` 即一例）。规则：stage 配置显式写全 env/algo_kwargs；已加守卫测试 `test_v3_config_overrides_every_forwarded_default_env_key`。
- **先跑常数策略奖励表，再训练。** `const_p` 的 mean reward/step 直接暴露奖励整形是否被"敞口税"主导（v2 满仓 −0.00076/步 vs alpha 量级 1e-4）。任何新奖励函数都应先过这一步。
- **年化常数必须由 timeframe 推导。** 硬编码 sqrt(2190) 让 1h 的 Sharpe 全部减半；用 `utils.metrics.bars_per_year(tf)`。
- **验证集 ≠ 留出集。** EvalCallback 选 best_model 用的片段不能再当 OOS 汇报；用日期钉死 `val_start`/`test_start`。v3 在 val 上"跑赢 b&h"，在 test 上却是所有策略中最差——没有留出集这个结论会完全相反。
- **累计型指标（OBV）的派生信号不是起点不变的。** 实时脚本用 400 天窗口重算，`sig_volume_obv_slope` 与训练管线差 2.0。任何在线推理脚本都要做与全历史管线的 `--verify` diff。
- **1h 上 PPO 的失败是特征问题，不是算法问题。** 线性基线 AUC 0.536 已经预示了；在同一信号池上继续调 RL 超参只会在 val 上 p-hacking。
- **shell：`pkill -f <pattern>` 会匹配到自己所在的 bash 命令行并把自己杀掉**（exit 144）。用 `pgrep` 先看，或用 `pkill -f "^python train.py"` 之类的锚定模式。

## 2026-09-06 · 多币种合并线性基线

- **"样本量不够"是假设，不是结论——先用监督基线证伪再动 PPO。** 四币合并 4 倍样本对 BTC AUC 只 +0.003~0.011，一小时就排除了多币种 PPO 这条路，省下的是几天训练。任何"加数据能救 RL"的想法先过 single / pooled / transfer 三臂对照。
- **合并多资产的 CV 必须按时间戳切折并加 embargo，不能按行切。** 同一时刻的 BTC/ETH 高度相关，索引切分会通过兄弟资产把验证期泄漏进训练。`utils.splits.time_folds` 已封装。
- **线性探针验不了样本量假设。** 10 参数的 LogReg 在 1.2 万样本上早已饱和，pooled 不涨是意料之中；要验"更多数据能否让非线性模型学到结构"必须同时跑一个小 GBDT。结果 GBDT 也不涨，结论才站得住。
- **分折看 regime。** 三折均值 0.53 掩盖了最近一折只有 0.51；而线上跑的正是最近这个 regime。汇报 AUC 时把最近一折单独列出。

## 2026-09-06 · H1 资金费率 IC 探针

- **预注册门槛不是形式。** funding 累计的 IC 均值 −0.07、最近一折 −0.16、分位价差 −171 bps，任何一个单看都会判"通过"；但中间一折（ETF 牛市）反号 +0.04，gate-on 下 +0.12。规则上线后遇到下一次强趋势就会系统性减仓。"三折同号 + 最近折不反号"这个门槛必须在看数据之前写死。
- **拥挤指标的方向取决于 regime。** funding 在震荡/熊市是反向指标，在强趋势里是延续指标。任何"极端 → 反向"假设都要分 regime 验证，并特别看"规则实际会触发的那个 regime"（gate-on 时段）——那里往往正是假设最不成立的地方。
- **事后加条件是 p-hacking 的入口。** gate-on 条件化是合理的分析，但它是第二次看数据；必须标记 post-hoc，结果无论好坏都不能当作"通过"。后续假设只能在前两折探索、最后一折留出。
- **"兼容性填充"要换成显式覆盖检查。** 四处静默 `fillna(0)` 让 6 个合约信号在 4h v2 里"训练期全零"而无人察觉；现在 `check_contract_coverage` 对信号池需要的数据源要求 ≥95% 覆盖，其余只打印。
- **时间计价的窗口必须由 timeframe 推导。** 合约信号的 `rolling(288)` 写死了 5min 假设，在 4h 上变成 48 天均值。凡是"24h 均值 / 1h 平滑"这种以时间定义的窗口，用 `pd.Timedelta(span) / pd.Timedelta(tf)` 换算，并保证不带 tf 的旧调用逐值不变。
- **底座先要能独立复现。** `gate_only` 此前只能作为 PPO 回测的副产品跑；`backtest_rules.py` 让它成为一等公民并精确复现 val +17.60% / test +17.21%，之后所有规则都以它为对照。
- **同一数据上第二个假设失败就停。** H1b 是对 H1 失败的"合理解释"，预注册后照样在同一折反号。这时最诱人的是提 H1c；规则是关闭这条线，把"为什么 f2 不一样"记为结构性观察（ETF 后 funding 由基差套利主导）而不是继续切子集。
- **过滤子集必须报 n。** gate-on 且 dist<5% 只剩 516 行，IC 动辄 ±0.5 还显示 PASS；探针现在每折 n<300 直接判不通过。任何条件化分析先看行数再看 IC。
- **信号通过 IC 门槛 ≠ 规则通过验收；先看回撤发生在哪。** H2a 的 OI 水平 z 三折同号、gate-on 下 IC −0.1，规则却一个基点回撤都没降——那段 −33% 是去杠杆下跌，OI z 全程为负。做"压回撤"的假设前，先把底座的最大回撤episode 标出来，看候选信号在那段是什么值；不在那段起作用的信号再好也是收益增强，不是风控。
- **回填要写到消费方读的那个库。** Spider 本机 .env 指向 localhost（192.168.1.111），Brain 读 192.168.1.68；第一次回填成功写了 20 万行到错的库。跨仓库的数据任务先对 DSN，再看行数。
- **Binance Vision 的 futures metrics 只有 daily 归档。** monthly URL 全部 404 但脚本"正常结束"，只补了 REST 30 天；回填器要把"0 行"当异常而不是"未上市"。
- **`pgrep -f` 会匹配到监视器自己的命令行。** 等待进程退出的 Monitor 因此永远不退出（和 lessons 里 pkill 那条同源）；匹配进程用模块名且排除自身，或改用 pid 文件。
- **验收标准要在留出段"有东西可改善"时才有意义。** test 段底座只有 −4.6% 回撤、0 笔交易，"回撤 ≤ 2/3 × 底座"对退出规则等价于不可能；写 §1.1 时没想到留出段可能是无事发生的四个月。风险规则在平静段的正确判定是"无害"，不是"更好"——但这个修订要在看到数据后明说、由用户拍板，不能自己顺手改。
- **ATR 单位的回撤止损在 4h/日线 BTC 上是噪声触发器。** 2×ATR(14) ≈ 5–8%，正常震荡天天碰到；减仓→反弹→加回的循环把 val 打成负收益。退出条件要用结构性水平（均线、N 日新低），不用波动率倍数。


## 2026-09-07 · H3'b 退出速度：V1 通过

- **先验变体各跑一次 + 独立区间确认，比在 val 上搜参数可信得多。** V1 是第 6 条在 val 上看过的规则，能说服人的不是 val 的 0.56，而是 2021-08→2024-10 这段从未参与选择的历史给出同方向的 0.68。任何"通过"都要再找一段没碰过的数据跑一次，且只跑一次。
- **退出信号触发就走完。** 减半（E1）回撤比 0.77，清仓（V1）0.56——留下的那一半仓位在 2025 上半年吃掉了大部分回撤。风控规则的"折中"往往两头不占。
- **四轮假设的顺序本可以倒过来。** 持仓数据（funding、OI）是"性感"的方向，但底座的最大回撤 episode 一看就是退出慢；如果第一步先标出回撤发生在哪、再问"什么信号在那段有值"，H1/H1b/H2 三轮可以省掉两轮。新原则：**做风控假设前先看底座回撤 episode 的时间线。**

## 2026-09-14 · read_position_ratio

- **带 `lru_cache` 的 engine 工厂在测试 fixture 里必须全部 monkeypatch。** `tests/test_contract_db.py` 只替换了 `get_engine`，而 `get_contract_engine` 也带缓存：第一个测试的 SQLite engine 被缓存，后面的读取器测试全读到空表——HEAD 上整文件跑就有 4 个失败，单独跑却能过。新增任何缓存工厂时同步改 fixture；新模块用 `db.get_contract_engine()` 运行时取，不要 `from utils.db import ...` 在导入期绑定。
- **给 DB 读取器传的 `since`/`until` 字符串要带时区。** psycopg2 会话时区不是 UTC 时，裸 `"2026-09-07"` 会被当本地时间解释（实测偏 8 小时）。沿用 `load_df_from_config` 的做法：先转成带 `+00:00` 的 ISO 再传。

## 2026-09-14 · V1 参数平台

- **一段未触碰区间的"同方向确认"不够，验收前先扫参数邻域和兄弟资产。** V1 在 pre_val 的 0.68 本来就没过 2/3，只是同方向；扫邻域发现只有 40/50 两档有效，60 起反而比不退出更差；ETH 上完全无效。这两个检验各花两分钟，本可以在 09-07 就做。
- **各资产最优参数不同 = 噪声。** BTC 40、ETH 30、SOL 50、BNB 100，这种"每个资产一个最优"就是网格搜索会给你的东西，只是规模小了一点。
- **回撤规则的有效性取决于回撤的形态，先按形态分类再评价。** 慢跌（2025-01，66 天）任何退出都行；震荡（2024-03）慢退出有害；急跌（2021-09，26 天）任何退出都没用。用一个 MDD 数字汇总三种形态会把结论平均掉。

## 2026-09-14 · H6 波动率目标

- **方向一致 ≠ 幅度够用。** H6 是第一个四资产全区间同向的候选，但主变体在任何一段都没到 2/3。预登记里把"方向"和"幅度"分成两条标准是对的，否则很容易把"真实但太弱"当成通过。
- **仓位类规则的收益代价要在登记时给出预期数字。** 登记时写了"平均仓位 0.6–0.8"，实际 ETH val 收益比 0.06、SOL pre_val 0.17，远超预期；下次登记仓位规则要先在底座上算一遍平均敞口和"平静上涨期少赚多少"，再定收益下限。
- **网格里的最优角只能进 A/B，不能进底座。** σ*=30%/N=30 在 BTC 和 ETH 都过线，但它是看完 9 组后才被挑中的；对它唯一诚实的检验是实盘。

## 2026-09-14 · H7 组合层

- **先算相关，再谈分散。** 四资产 gate_only 收益相关 0.5–0.8，回撤期趋近 1；这一个表就能预告分散压不住回撤。以后任何"组合层"假设，第一步是相关矩阵，第二步才是回测。
- **一条目标线的三次预登记失败比一次"通过"更有价值。** 退出速度、波动率目标、分散三条路都按同一流程关闭，"接受底座回撤水平"这个结论现在是有证据的决定，而不是放弃。

## 2026-09-17 · H8 震荡感知退出

- **"分阶段处理"要先问：阶段是事后标签还是事前可算？** 事后标签（拆 episode 看形态）只能用来理解，进规则就是偷看未来。事前可算的版本必然滞后，滞后的切换往往正好落在下一个阶段的开头，比不切换更差。H8 在 18 格里有 9 格比不退出还差。
- **机制检验先于全量回测能省一轮。** 日线复现 2 分钟就看出"2024 震荡段 H8 与 V1 逐一相同"，全量回测只是确认。以后凡是针对某段 episode 提出的规则，先在那段 episode 内跑复现。
- **改进型假设要和被改进的规则并排比，不只和底座比。** H8 的对照必须是 V1；只看 gate_only 比值 0.68 会误以为"有效"。

## 2026-09-17 · H4/H5 持仓信号探针

- **合并外部序列要按 K 线的时区切桶。** 比率按 UTC 切日、K 线按上海时区切日，错开 8 小时，合并后全 NaN。覆盖检查按设计拦下了（0.0% 报错），说明"不填零 + 覆盖门槛"这两条规则值。
- **探针的 PASS 要和登记方向一起看。** 1d 全样本 `retail_minus_top` 三折同号被标 PASS，但方向与登记相反；工具只检查同号，不检查方向，判定时要人工核对。
- **同一结构性观察在两个独立数据源出现，比通过一个 IC 门槛更值得记。** funding（H1）与散户/大户分歧（H4b）都在 2023-11 后反号，指向同一件事：ETF 后拥挤指标变成延续指标。

## 2026-09-17 · H4b' 兄弟资产

- **单资产上再强的 IC 也不是证据，兄弟资产是必做的。** BTC 最近折 +0.31 看起来像本项目最强的信号，BNB 同时段 −0.23。没有回填其他币种就会把它登记成"结构性效应"。
- **归档起点不同时，覆盖门槛要区分"起点前缺失"和"内部缺口"。** 兄弟资产从 2021-12 才有数据，覆盖 74% 全部来自起点前；用 `--min-ratio-coverage` 显式放宽并写明原因，而不是改默认值。
- **并行重活会被内存看门狗杀掉。** 回填 + 两个探针同时跑被杀两次；探针峰值 3.6 GB，改前台单跑就好。

## 2026-09-21 · A 股 E-A1 / H-A1

- **跨市场先验不等于可迁移。** SMA200 门控在四个加密资产上成立，在 A 股等权指数上收益只剩 0.3–0.45，因为 A 股等权的收益集中在少数 V 形反弹（2019-Q1、2022-07），趋势门控结构性错过。预判写"通过"是把先验当成了结论；下次预判要先看目标资产的收益分布形态（集中度）。
- **先看零成本版本，再看净值。** 一次诊断就把"踏空"和"换仓成本"分开了（pre_val 0.45 vs 0.30）；不做这一步会误以为是成本模型的问题。
- **宽表构建的三个坑**：数据起点 ≠ 上市日（看窗口前 20 日）；停牌率分母要用当日有记录的代码；0 价格要置 NaN。`date_trunc` 按会话时区切月，2015-12 的桶其实是 2016-01。
- **固定金额流动性门槛随年份漂移。** 5000 万在 2017 年只留 1/3 宇宙；相对排名规则（剔除后 10%）才是稳定的宇宙定义。

## 2026-09-21 · A 股 E-A2/E-A3/H-A3b

- **先问基准里已经含了什么。** 月度再平衡等权基准每月卖涨买跌，本身就是一个反转策略；在它上面叠加反转，2022 年后只剩 1.06 倍。定目标函数时应该先算"基准与候选信号的重叠"，否则一天的工作在验证一件基准定义已经决定的事。
- **收益层验收里"MDD 不深于基准"是硬约束，且对集中组合天然不利。** 随机十分位对照能把"集中"和"信号性质"分开，成本 2 分钟，应成为 L2 的固定对照。
- **IC 门槛之外的十分位价差条款救了一次。** 低波动 IC 三折同号 t≥2，但两端非单调；没有价差条款会把它判通过。

