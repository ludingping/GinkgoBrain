...
help:
	...
	@echo ""
	@echo "  install        初始化虚拟环境并安装依赖（uv sync）"
	@echo "  sync           同步依赖到 uv.lock（依赖变更后执行）"
...
install:
	...
	@echo "Or run directly:    uv run python train.py ..."
...
train-stock:
	uv run python train.py --mode stock --config $(CONFIG) $(if $(RUN),--run-name $(RUN),)

train-crypto:
	uv run python train.py --mode crypto --config $(CONFIG) $(if $(RUN),--run-name $(RUN),)
...
ifndef MODEL
	...
	uv run python evaluate.py --mode stock --model $(MODEL) --config $(CONFIG) --episodes $(EPISODES)
